...
When a transaction [Bitcoin](##)is created, it is first transmitted to all nodes in the network. These nodes verify that the[transaction](https://coinacademy.fr/academie/transaction-blockchain/)is valid (e.g. that the sender has enough bitcoins to complete the transaction) and, if so, they add it to their mempool.
